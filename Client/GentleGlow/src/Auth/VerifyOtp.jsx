...
const VerifyOtp = () => {
  ...
  const [otpDigits, setOtpDigits] = useState(["", "", "", ""]); // Initialize with empty strings
  const inputRefs = useRef([]); // Refs to hold input field references
  ...
  const handleOtpChange = (index, value) => {
    ...
    // Move focus to the next input field if not empty
  ...
  const VerifyUser = async (e) => {
    ...
    if (otpcode && useremail) {
      ...
      console.log(otpnum)
      if (otpnum === otpcode) {
        SuccessToast("Verified", 3000);
        navigate("/forgetpass")
        
      } else {
        ErrorToast("Otp Not Verified")
      }
            // const UserOtp = await axios.post("/api/otp", { otpnum , useremail });
